...
class FindTest < Minitest::Test
  ...
  def test_no_waldo
    ...
    found = words.find(-> {"not here"}) do |word|
      word == "waldo"
    end
    assert_equal "not here", found
  end
  ...
  def test_find_first_even_number
    ...
    found = numbers.find { |n| n.odd? == false }
  ...
  def test_find_first_word_ending_with_er
    ...
    p = Proc.new { |x| x[-2, 2] == "er" }
    found = words.find { |word| p.call(word) }
