...
class RejectPatternTest < Minitest::Test
  ...
  def test_remove_dinosaurs
    ...
    animals.each do |item|
      notasaurus << item unless (/saurus/).match(item)
    end
  ...
  def test_remove_numbers
    ...
    elements.each do |item|
      not_numbers << item unless item.is_a?(Numeric)
    end
  ...
  def test_remove_floats
    ...
    not_numbers = []
    elements.each do |item|
      not_numbers << item unless item.is_a?(Float)
    end
  ...
  def test_remove_animals_starting_with_vowels
    ...
    animals.each do |animal|
      remaining << animal unless animal[0].match(/[aeiouy]/)
    end
  ...
  def test_remove_arrays
    ...
    remaining = []
    elements.each do |item|
      remaining << item unless item.is_a?(Array)
    end
  ...
  def test_remove_hashes
    ...
    remaining = []
    elements.each do |item|
      remaining << item unless item.is_a?(Hash)
    end
